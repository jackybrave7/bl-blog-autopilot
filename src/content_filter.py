...
def _keyword_matches(kw: str, blob: str) -> bool:
    """Latin keywords match whole tokens; Cyrillic phrases keep substring match."""
    if re.search(r"[a-z]", kw):
        return re.search(r"(?<![a-z])" + re.escape(kw) + r"(?![a-z])", blob) is not None
    return kw in blob
...
def find_blocked_topic(*texts: str) -> tuple[str, str, str] | None:
    ...
    for category, meta in load_stop_topics().items():
        ...
        for keyword in meta.get("keywords", []):
            ...
            if kw and _keyword_matches(kw, blob):
